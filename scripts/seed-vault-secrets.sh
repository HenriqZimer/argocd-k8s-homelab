...
seed_vaultwarden_credentials() {
  ...
  local admin_token="${VAULTWARDEN_ADMIN_TOKEN:-}"
  local smtp_username="${VAULTWARDEN_SMTP_USERNAME:-}"
  local smtp_password="${VAULTWARDEN_SMTP_PASSWORD:-}"

  if [[ -z "${username}" ]]; then
    username="$(secret_value secrets vaultwarden-credentials username)"
    password="$(secret_value secrets vaultwarden-credentials password)"
  fi
  [[ -n "${admin_token}" ]] || admin_token="$(secret_value secrets vaultwarden-credentials admin_token)"
  [[ -n "${smtp_username}" ]] || smtp_username="$(secret_value secrets vaultwarden-credentials smtp_username)"
  [[ -n "${smtp_password}" ]] || smtp_password="$(secret_value secrets vaultwarden-credentials smtp_password)"

  [[ -n "${username}" ]] || username="vaultwarden"
  [[ -n "${admin_token}" ]] || admin_token="$(openssl rand -hex 32)"
  [[ -n "${smtp_username}" ]] || smtp_username="dummy@gmail.com"
  [[ -n "${smtp_password}" ]] || smtp_password="dummy-app-password"

  if [[ -z "${password}" ]]; then
    echo "Skipping vaultwarden-credentials: no password found and VAULTWARDEN_DB_PASSWORD is not set." >&2
    return
  fi

  vault_kv_put vaultwarden-credentials "$(
    jq -n \
      ...
      '{"username":$username, "password":$password, "admin_token":$admin_token, "smtp_username":$smtp_username, "smtp_password":$smtp_password}'
  )"
}
